...
train = pd.read_csv("./tempData/archive/emnist-bymerge-train.csv")
test = pd.read_csv("./tempData/archive/emnist-bymerge-test.csv")
...
for i in range(len(test)):
   test_data.append([test[i], test_labels[i]])
...
class SimpleResidualBlock(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv2d(in_channels=784, out_channels=47, kernel_size=3, stride=1, padding=1)
        self.relu1 = nn.ReLU()
        self.conv2 = nn.Conv2d(in_channels=784, out_channels=47, kernel_size=3, stride=1, padding=1)
        self.relu2 = nn.ReLU()
...
model = to_device(ResNet9(784, 47), device)
# Main evaluator
@torch.no_grad()
